...
SELECT *,
	RANK() OVER(ORDER BY A4 DESC) AS Rank_inhabitants
FROM bank.district;

SELECT *,
	RANK() OVER(ORDER BY A9 DESC) AS no_cities
FROM bank.district;
...
SELECT *,
	RANK() OVER(PARTITION BY A3 ORDER BY A4 DESC) AS Rank_inhabitants_region
FROM bank.district;


SELECT *,
	RANK() OVER(PARTITION BY A3 ORDER BY A9 DESC) AS no_cities
FROM bank.district;
