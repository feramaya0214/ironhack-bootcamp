...
SELECT account_id, COUNT(*) AS 'Number of transactions' 
FROM bank.trans
GROUP BY account_id
ORDER BY account_id ASC;

select avg(tt.count_trans) from (select account_id, count(*) as 'count_trans' from trans group by account_id) as tt;
...
SELECT * FROM bank.account WHERE district_id IN ( SELECT A1 FROM bank.district WHERE A3 = 'Central Bohemia' );

-- 2. Rewrite the previous as a join query.

SELECT a.* FROM bank.account a JOIN bank.district d ON a.district_id = d.A1 WHERE d.A3 = 'Central Bohemia';
